...
void write_change_to_file(std::string file_name, std::map< std::string, Student* >& user_ids) {

    std::ofstream printer;
    printer.open(file_name);

    for (auto pair : user_ids) {
        std::string line_to_write = pair.second->student_number + ";" +
                pair.second->user_id + ";" +
                pair.second->name + ";" +
...
void change_phone_number(std::string number, std::string file_name, std::map< std::string, Student* >& user_ids, std::map< std::string, Student* >& student_numbers) {

    bool number_found = false;

    for (auto pair : user_ids) {
        if (pair.second->student_number == number) {
            number_found = true;
        }
    }
    ...
    for (auto pair : user_ids) {
        if (pair.second->student_number == number) {
            pair.second->phone_number = new_number;
        }
    }

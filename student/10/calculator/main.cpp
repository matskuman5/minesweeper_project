...
int main() {
    ...
    while ( true ) {
        ...
        for (Command c : COMMANDS) {
            if (c.str == command_to_be_executed) {
                ...
                if (c.parameter_number == 2) {

                    double par1;
                    double par2;

                    if (!string_to_double(pieces.at(1), par1) || !string_to_double(pieces.at(2), par2)) {
                        cout << "Error: a non-number operand." << endl;
                        continue;
                    }

                    cout << c.action(par1, par2) << endl;
